...
- **Frontend: дробные границы NUMBER на /fields (26.09.2026)** — выполнено:
  - тип `NUMBER` допускает целые и дробные значения на всём пути: API
    валидирует `meta_data.min/max` через `_is_number` (int или float),
    бот парсит ввод `parse_number` («5» → int, «2.5»/«2,5» → float),
    ответ хранится унифицированно строкой в `value_text`;
  - на странице `/fields` инпуты границ `min`/`max` были `type="number"`
    без `step` — HTML-дефолт `step="1"` отклонял дробные значения
    («Минимум не может быть больше максимума» не спасал: браузер не
    давал ввести 2.5); форма с `@submit.prevent` без `novalidate`,
    т.е. блокировка нативная;
  - исправление: `step="any"` на обоих инпутах (`fld-min`, `fld-max`);
    backend и парсер бота не менялись.
